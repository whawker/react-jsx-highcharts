...
describe('<Title />', () => {
  ...
  describe('update', () => {
    it('should use the setTitle method when the data changes', () => {
      ...
      expect(testContext.chartStubs.setTitle).toHaveBeenCalledWith({ x: 10, y: 20, text: 'My New Title' }, null, true);
    });
  });
  ...
  describe('when unmounted', () => {
    it('removes the title by setting the title to text', () => {
      ...
      wrapper.unmount();
      expect(testContext.chartStubs.setTitle).toHaveBeenCalledWith({ text: null }, null, true);
    });
  });
});
