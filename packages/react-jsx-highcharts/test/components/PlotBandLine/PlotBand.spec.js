...
describe('<PlotBand />', () => {
  ...
  describe('when mounted', () => {
    ...
    it('uses the provided ID if id prop is a string', () => {
      mount(<PlotBand id="myPlotBandIdStr" from={1} to={2} />);
    ...
    it('resolves the ID if id prop is a function', () => {
      ...
      mount(<PlotBand id={idFunc} from={1} to={2} />);
      expect(testContext.axisStubs.addPlotBandOrLine.mock.calls[0][0].id).toBe(
  ...
  describe('when unmounted', () => {
    it('removes the plot band by id (if the parent axis still exists)', () => {
      const wrapper = mount(<PlotBand id="My PlotBand" from={1} to={2} />);
