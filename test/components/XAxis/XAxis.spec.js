...
describe('<XAxis />', function ()  {
  ...
  it('renders an <Axis />', function ()  {
    const wrapper = shallow(<XAxis id="myAxisAxis" getChartType={this.isHighchartsChart} />);
  ...
  it('renders the <Axis /> type if provided (Highcharts chart)', function () {
    const wrapper = shallow(<XAxis id="myAxisAxis" getChartType={this.isHighchartsChart} type="logarithmic" />);
    expect(wrapper).to.have.prop('type').equal('logarithmic');
  });

  it('renders the <Axis /> type if provided (Highstock chart)', function () {
    const wrapper = shallow(<XAxis id="myAxisAxis" getChartType={this.isHighstockChart} type="logarithmic" />);
    expect(wrapper).to.have.prop('type').equal('logarithmic');
  });
  ...
  it('renders the an <Axis type="datetime" /> if no type specified (Highstock chart)', function () {
    const wrapper = shallow(<XAxis id="myAxisAxis" getChartType={this.isHighstockChart} />);
    expect(wrapper).to.have.prop('type').equal('datetime');
  });

  it('passes other props through to <Axis />', function () {
    const wrapper = shallow(<XAxis id="myAxisAxis" getChartType={this.isHighchartsChart} tickLength={1337} />);
